...
def test_convert_base_temperature():
    # Test if base temperature conversion works
    base_temp_f = 65
...
def test_fetch_temperature_data():
    # Fetch test data from Meteostat for a known location (Berlin) and time range
    ...
    # Check if the DataFrame has been fetched and contains the expected columns
...
if __name__ == '__main__':
    test_convert_base_temperature()
    test_calculate_mean_temperature()
    test_calculate_degree_days_hdd()
    test_calculate_degree_days_cdd()
    test_fetch_temperature_data()
